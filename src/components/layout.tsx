...
import Camera,
{ getCameraOrder } from "../components/media/camera";
import {
    LIVE_LINK,
    LocalSessionContext,
} from "../entry";
...
const TABS = [
    {
        icon: <PeopleIcon
            role="img"
            size="1.5rem" />,
        title: `title_participants`,
        menuType: ClassroomMenuType.ForAll,
    },
    {
        icon: <LessonPlanIcon
            role="img"
            size="1.5rem" />,
        title: `title_lesson_plan`,
        menuType: ClassroomMenuType.ForHostOnly,
    },
    {
        icon: <ChatIcon
            role="img"
...
const OPTION_COLS_OBSERVE = [
    {
        id: `option-numcol-2`,
        title: <FormattedMessage id="two_columns" />,
        value: 2,
    },
    {
        id: `option-numcol-4`,
        title: <FormattedMessage id="four_columns" />,
        value: 4,
    },
    {
        id: `option-numcol-6`,
        title: <FormattedMessage id="six_columns" />,
        value: 6,
    },
];
        ...
        return (
            ...
            >
                <React.Suspense fallback={<Typography variant="body2"><Skeleton variant="text" /></Typography>}>
                    <Grid
                        item
                        className={clsx(classes.scrollContainer, classes.messageContainer)}>
            ...
            >
                ...
                <Grid
                    ...
                    <FormControl style={{
                        ...
                        >
                            {OPTION_COLS_OBSERVE.map((option) => <MenuItem
                                key={option.id}
                                ...
                                <Grid
                                    ...
                                    >
                                        <Grid item>
                                            ...
                                            >
                                                {isHostTeacher ?
                                                    TABS.filter((t) => t.menuType !== 1).map((tab, index) => <StyledTab
                                                        key={`tab-button-${tab.title}`}
                                                        className={index === drawerTabIndex ? classes.tabSelected : ``}
                                                        title={tab.title}
                                                        value={index}>{tab.icon}</StyledTab>) :
                                                    TABS.filter((t) => t.menuType !== 0).map((tab, index) => <StyledTab
                                                        key={`tab-button-${tab.title}`}
                                                        className={index === drawerTabIndex ? classes.tabSelected : ``}
                                                        title={tab.title}
                                                        value={index}>{tab.icon}</StyledTab>)
                                ...
                                <Grid
                                    ...
                                    <SessionsContext.Provider value={sessions}>
                                        <MessageContext.Provider value={messages}>
                                            {isHostTeacher ?
                                                TABS.filter((t) => t.menuType !== 1).map((tab, index) => <TabPanel
                                                    key={`tab-panel-${tab.title}`}
                                                    index={index}
                                                    tab={tab}
                                                    value={drawerTabIndex} />) :
                                                TABS.filter((t) => t.menuType !== 0).map((tab, index) => <TabPanel
                                                    key={`tab-panel-${tab.title}`}
                                                    index={index}
                                                    tab={tab}
                                                    value={drawerTabIndex} />)
                            ...
                            <Grid
                                ...
                                <Collapse in={drawerOpen}>
                                    <Grid
                                        ...
                                        xs={12}
                                        style={{
                                            backgroundColor: theme.palette.type === `light` ? `#FFF` : `#030D1C`,
                                        }}>
                                        <SessionsContext.Provider value={sessions}>
                                            <MessageContext.Provider value={messages}>
                                                {isHostTeacher ?
